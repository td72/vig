...
mod tests {
    ...
    fn git_name_map() -> HashMap<&'static str, usize> {
        use crate::git::state::{
            PANE_BRANCH_LIST, PANE_DIFF_VIEW, PANE_FILE_TREE, PANE_GIT_LOG, PANE_REFLOG,
        };
    ...
    fn github_name_map() -> HashMap<&'static str, usize> {
        use crate::github::state::{
            GH_PANE_ISSUE_DETAIL, GH_PANE_ISSUE_LIST, GH_PANE_PR_DETAIL, GH_PANE_PR_LIST,
        };
        let mut m = HashMap::new();
    ...
    #[test]
    fn load_github_page() {
        let cfg = load_github_page_config(&github_name_map()).unwrap();
