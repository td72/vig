...
ポーリングする他のツールも同じ上限を消費します。Projects ページを開いた
ままの vig は 5 分ごとにボードを再取得するため、忘れられたターミナルの
タブが一日中ポイントを消費します。GraphQL の残りポイント（5,000/時）が
1,500 を切ると Projects のステータスバーに `⚠ api N left` と警告が
出ます。
