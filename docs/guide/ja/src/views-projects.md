...
押すと隣のレーンへ移動します。Roadmap レイアウトのビュー準拠描画は
[#148](https://github.com/td72/vig/issues/148) の残りの sub-issue で進行中
です。
...
- `gh project` にはトークンの `project` スコープが必要です。無い場合は
  ...
  実行してから `r` を押してください。
- ボードはまず `--limit 100` で取得し、それより多いときだけ
  `--limit 500` で 1 回だけ再取得します（GraphQL のコストは要求 limit に
  比例するため、小さいボードは安いままです）。500 を超える分は
  ステータスバーに `(truncated)` と出ます。
- アカウントの GraphQL クォータ（5,000 ポイント/時）の残りが 1,500 を
  切るとステータスバーに `⚠ api N left` と警告します —
  [トラブルシューティング](troubleshooting.md#-github-rate-limited)参照。
