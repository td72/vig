...
| `y` | アイテムの URL をコピー |
| `x` | 一覧に closed な issue / マージ済み・closed な PR を表示 / 非表示（初期状態は `github-show-closed`） |
| `Esc` | 一覧に戻る |
...
Markdown ファイル (`.md` / `.markdown`) はプレビューでレンダリングされます —
見出し・強調・リスト・コード・GFM テーブル（ペイン幅に収まるよう調整）。
`m` で生テキスト（ハイライト表示）に切り替えられ、`markdown-preview "raw"`
で生テキストを既定にできます。
...
| `/` `n` `N`（プレビュー） | ファイル内容を検索（一致をハイライトし、表示が追従） |
...
GraphQL で取得 — 1 回あたり数ポイント）。ページ表示中は `projects-poll-interval`（30 秒、
プロジェクトの `updatedAt` を読む 1 ポイントのプローブ）ごとに変更を確かめ、変わったときだけ
ボードを再取得します。ボードは全幅で表示され、
