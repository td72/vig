...
上部の System ペインはマシン全体の値をグラフ表示します。グローバル CPU %（直近ピーク付き）と
使用メモリを直近 `procs-history` サンプル分（デフォルト 120 = 2 秒間隔で 4 分）のスパークラインで
描き、スワップがあればその行も表示します。`c` で CPU グラフをコアごとのバー表示に切り替えられます。
グラフはバッファが埋まるまで右詰めで伸び、ビュー表示中のみサンプリングされ、常にマシン全体が
対象です（数値のみを描画します）。詳細ペインには選択中プロセスの CPU % / 常駐メモリの履歴
スパークラインが CPU / MEM 欄の下に表示されます。
