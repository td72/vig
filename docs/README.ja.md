...
- **Projects View** — GitHub Projects (v2) のボード。`Status` ごとのカンバン列、ソートできるテーブルモード、プロジェクトフィールドをすべて表示するアイテム詳細。`gh project` 使用（読み取り専用）
...
| `Tab` / `Shift+Tab` | ペイン切り替え: Processes → Ports → Detail |
...
GitHub Projects (v2) を読み取り専用のボードとして眺めるビューです。`gh project list` /
`gh project field-list` / `gh project item-list --format json` だけを使います。左のペインは
リポジトリのオーナー（ユーザーまたは組織）のオープンなプロジェクト一覧で、現在のリポジトリに
リンクされたプロジェクトが `▸` 付きで先頭に並びます。プロジェクトを選ぶとボードを読み込みます。
列は `Status` の選択肢を GitHub 上の順に 1 列ずつ、加えてステータス未設定のアイテム用の
`No status` 列です。カードにはアイテム種別（`●` issue、`⇅` pull request、`✎` draft）、番号、
タイトル、担当者を表示します。`t` でテーブルモードに切り替わり、1 行 1 アイテムで
...
| キー | 操作 |
|------|------|
| `j` / `k`（プロジェクト） | 選択移動（ボードが追従） |
| `Enter` / `i` / `l`（プロジェクト） | ボードにフォーカス |
...
| `h` / `Esc`（詳細） | ボードに戻る |
| `Esc`（ボード） | プロジェクト一覧に戻る |
| `Tab` / `Shift+Tab` | ペイン切り替え: Projects → Board → Detail |
| `/` `n` `N` | 検索（プロジェクト名、または列をまたいだアイテムのタイトル / 番号） |
| `r` | プロジェクト一覧・ボード・表示中アイテムを再取得 |
